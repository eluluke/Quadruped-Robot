"""
remote_locomotion_v3.py

Automatic sequence + Xbox-controlled trajectory:
    ...
    4. Build selected trajectory table from trajectory_v3.py.
    5. Move to selected trajectory start phase.
    6. Use Xbox left stick Y to control gait phase speed.

This combines:
    - the successful stand_then_modular_locomotion_v4_swing_first.py flow
    - the Xbox phase-control architecture from remote_locomotion_v2.py

Before running:
    Put the leg physically at the known max-contraction / mechanical-limit pose.

Run:
    python3 remote_locomotion_v3.py -c can1

Trajectory switching:
    Change TRAJECTORY_NAME:
        "regular_planar"
        "tilted_planar"
        "vertical_jump"

For regular planar:
    COMMAND_HIP_TRAJECTORY = False is recommended to hold hip and reduce sag.

For tilted_planar:
    COMMAND_HIP_TRAJECTORY = True may be needed because y changes and hip should move.
"""
...
TRAJ_CFG = TrajectoryConfig(
    ...
    # Your corrected working planar signs.
    x_forward_sign=1.0,
    y_sideways_sign=1.0,
    z_lift_sign=-1.0,
...
# "swing_start" = first visible gait motion is cycloidal swing forward.
# "neutral_mid_stance" = starts from neutral/mid-stance.
...
try:
    print("=" * 80)
    print("remote_locomotion_v3.py - stand then Xbox modular trajectory")
    print("=" * 80)
    print("Stage 1: max-contraction -> neutral using relative IK deltas")
    print("Stage 2: Xbox controls trajectory phase speed")
    print()
    ...
    if RUN_TRAJECTORY_AFTER_STANDING and AUTO_START_TRAJECTORY_AFTER_HOLD:
        ...
        start_index = table_index_from_phase(phase, len(table))
        ...
        print(f"Moving to trajectory start phase {phase:.3f}...")
        set_all_gains("run")
        move_all_to_targets(
            first_targets,
            move_time=0.6,
        ...
        print("\nStarting Xbox-controlled modular trajectory.")
