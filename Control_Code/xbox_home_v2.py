...
#          hip   -> ID 1
#   2. Run this script.
#   3. It first puts all joints into IDLE and waits briefly to reduce CAN noise.
#   4. It reads startup raw positions while IDLE, then enters POSITION mode softly.
#   5. Type a joint ID, e.g. 3, then press Enter.
#   6. Use Xbox left stick Y to move that joint slowly.
#   7. Release joystick at the mechanical limit. The joint will hold.
#   8. Type y + Enter to capture this joint's homing offset.
#   9. Repeat for all joints.
#   10. Type write + Enter to save homing_offsets.py.
#
# Offset convention:
...
def put_all_motors_idle_without_stopping_bus(reason=""):
    """
    Put all motors into IDLE without stopping the CAN bus.

    This is used at startup before any controlled homing motion. It reduces
    motor command traffic/noise while still keeping the bus available for
    measured-position reads and later position-mode arming.
    """
    if reason:
        print(f"\nPutting all motors into IDLE first: {reason}")
    else:
        print("\nPutting all motors into IDLE first...")
    ...
    # Let old mode-change / feedback frames settle before reading startup pose.
    time.sleep(0.35)
...
def initialize_position_hold():
    ...
    # Before arming position mode, put everything in IDLE and let the bus settle.
    # This avoids beginning the program with three joints already broadcasting /
    # holding / responding in a noisy command state.
    put_all_motors_idle_without_stopping_bus(
        "startup quiet period before reading initial positions"
    )

    print("\nReading startup measured positions while motors are IDLE...")
