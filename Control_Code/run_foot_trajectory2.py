...
# --------------------------------
def sync_reference(motor_id, sync_time=0.25):
    ...
    if current_pos is None:
        raise RuntimeError(f"Failed to sync joint {motor_id}")
...
# Hip stays in damping and does not block startup
# --------------------------------
def get_stable_start_positions():
    print("Entering damping mode for all joints...")

    for motor_id in [HIP_ID, THIGH_ID, SHANK_ID]:
    ...
    while True:
        ...
        if time.perf_counter() - t_start > MAX_STARTUP_TIME:
            raise RuntimeError(
                "Startup not stable "
                f"(hip_vel={hip_vel}, "
                f"thigh_vel={thigh_vel}, "
                f"shank_vel={shank_vel})"
            )
...
# foot points -> your IK -> motor commands
# --------------------------------
def build_command_table(start_thigh_angle, start_shank_angle):
    num_points = int(CYCLE_TIME * RATE_HZ)

    x0, y0, z0 = foot_trajectory(0.0)
    theta_h_ref0, theta_t_ref0, theta_s_ref0 = leg_ik(x0, y0, z0)

    print(
        f"Ref angles: hip={theta_h_ref0:.3f}, "
        f"thigh={theta_t_ref0:.3f}, shank={theta_s_ref0:.3f}"
    )
    ...
    for point_index in range(num_points):
        ...
        _, theta_t_des, theta_s_des = leg_ik(
            x_des,
            y_des,
            z_des,
        )

        cmd_thigh = start_thigh_angle + (
            theta_t_des - theta_t_ref0
        ) * GEAR_RATIO

        cmd_shank = start_shank_angle + (
            theta_s_des - theta_s_ref0
        ) * GEAR_RATIO
...
try:
    ...
    for motor_id in [THIGH_ID, SHANK_ID]:
        set_gains(
            motor_id,
            STARTUP_KP,
            STARTUP_KD,
    ...
    print("Starting discrete foot trajectory with your IK...")
    counter = 0
    point_index = 0
