"""
remote_locomotion_v1.py

Xbox-controlled one-leg locomotion using trajectory_v1.py.

Left stick Y controls gait phase speed.
Trajectory geometry and IK table logic are imported from trajectory_v1.py.

Future 4-leg direction:
    leg_FL phase = phase
    leg_RR phase = phase
    leg_FR phase = phase + 0.5
    leg_RL phase = phase + 0.5
with each leg on a separate CAN bus: can1, can2, can3, can4.
"""
...
MAX_PHASE_SPEED = 0.80        # cycles/sec at full stick
PHASE_ACCEL_LIMIT = 4.0       # cycles/sec^2
ALLOW_REVERSE = True
FREEZE_PHASE_WHEN_STOPPED = True

# Starts near neutral stance: for stance_ratio=0.5, phase=0.25 gives forward ~= 0, lift=0.
...
def smooth_move_to_phase(start_raw_by_id, table, phase):
    index = table_index_from_phase(phase, len(table))
    targets = raw_targets_by_id_from_start_raw(
        start_raw_by_id, ROLE_TO_ID, table[index], COMMAND_ROLES
    )
    print(f"\nMoving to initial phase {phase:.3f}...")
    steps = int(MOVE_TO_START_TIME * RATE_HZ)
...
try:
    print("=" * 80)
    print("remote_locomotion_v1.py - Xbox modular one-leg trajectory runner")
    print("=" * 80)
    ...
    phase = START_PHASE
    smooth_move_to_phase(start_raw_by_id, table, phase)
    ...
    while not STOP_REQUESTED:
        ...
        ly_filtered = (1.0 - JOYSTICK_FILTER_ALPHA) * ly_filtered + JOYSTICK_FILTER_ALPHA * ly
